...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    @Query("SELECT CASE WHEN COUNT(b) > 0 THEN true ELSE false END FROM Booking b " +
            "WHERE b.item.id = :itemId AND b.booker.id = :bookerId AND b.end < :now AND b.status = :status")
    boolean existsByItemIdAndBookerIdAndEndBeforeAndStatus(
            @Param("itemId") Long itemId,
            @Param("bookerId") Long bookerId,
            @Param("now") LocalDateTime now,  // было "end", должно быть "now"
            @Param("status") BookingStatus status
    );

    List<Booking> findByItemIdAndBookerIdAndStatus(Long itemId, Long userId, BookingStatus bookingStatus);
}
